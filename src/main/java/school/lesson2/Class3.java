package school.lesson2;

import java.util.Arrays;

public class Class3 {
    public static void main(String[] args) {
        method5();
        method6();
        method7();
        method8();
        method9(5,8);
    }

    // 5.    Задать целочисленный массив, состоящий из элементов 0 и 1. Например: [ 1, 1, 0, 0, 1, 0, 1, 1, 0, 0 ]. С помощью цикла и условия заменить 0 на 1, 1 на 0;
    public static void method5() {
        int[] nums = {1, 1, 0, 0, 1, 0, 1, 1, 0, 0};
        for (int i = 0; i < nums.length; i++) {
            if (nums[i] == 1) nums[i] = 0;
            else nums[i] = 1;
        }
        System.out.println(Arrays.toString(nums));
    }

    // 6.    Задать пустой целочисленный массив длиной 100. С помощью цикла заполнить его значениями 1 2 3 4 5 6 7 8 … 100;
    public static void method6() {
        int[] mass = new int[100];
        for (int i = 0; i < mass.length; i++) {
            mass[i] = i + 1;
            System.out.print(mass[i] + " ");
        }
    }

    // 7.    Задать массив [ 1, 5, 3, 2, 11, 4, 5, 2, 4, 8, 9, 1 ] пройти по нему циклом, и числа меньшие 6 умножить на 2;
    public static void method7() {
        int[] mass2 = {1, 5, 3, 2, 11, 4, 5, 2, 4, 8, 9, 1};
        for (int i = 0; i < mass2.length; i++) {
            if (mass2[i] < 6) mass2[i] = mass2[i] * 2;
        }
        System.out.println();
        System.out.println(Arrays.toString(mass2));
    }

    /*8.    Создать квадратный двумерный целочисленный массив (количество строк и столбцов одинаковое),
     и с помощью цикла(-ов) заполнить его диагональные элементы единицами (можно только одну из диагоналей,
     если обе сложно). Определить элементы одной из диагоналей можно по следующему принципу: индексы таких
     элементов равны, то есть [0][0], [1][1], [2][2], …, [n][n];*/
    public static void method8() {
        int[][] mass3 = new int[6][6];
        for (int i = 0; i < mass3.length; i++) {
            for (int j = 0, x = mass3[i].length; j < mass3.length; j++, x--) {
                if (i == j || i == x - 1) mass3[i][j] = 1;
                System.out.print(mass3[i][j] + "");

            }
            System.out.println("\n");
        }
    }

    /*9.  Написать метод, принимающий на вход два аргумента: len и initialValue, и возвращающий одномерный
    массив типа int длиной len, каждая ячейка которого равна initialValue;*/

    public static void method9(int len, int initialValue) {
        int[] mass4 = new int[len];
        for (int i = 0; i < mass4.length; i++) {
            mass4[i] = initialValue;
        }
        System.out.println(Arrays.toString(mass4));
    }

}


